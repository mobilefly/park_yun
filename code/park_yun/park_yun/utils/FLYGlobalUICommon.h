//
//  CONST.h
//
//  Created by chen on 14-6-7.
//  Copyright (c) 2014年 无线飞翔. All rights reserved.
//

#import <Foundation/Foundation.h>

#define FLY_ROW_HEIGHT                 FLYkDefaultRowHeight
#define FLY_TOOLBAR_HEIGHT             FLYkDefaultPortraitToolbarHeight
#define FLY_LANDSCAPE_TOOLBAR_HEIGHT   FLYkDefaultLandscapeToolbarHeight

#define FLY_KEYBOARD_HEIGHT                 FLYkDefaultPortraitKeyboardHeight
#define FLY_LANDSCAPE_KEYBOARD_HEIGHT       FLYkDefaultLandscapeKeyboardHeight
#define FLY_IPAD_KEYBOARD_HEIGHT            FLYkDefaultPadPortraitKeyboardHeight
#define FLY_IPAD_LANDSCAPE_KEYBOARD_HEIGHT  FLYkDefaultPadLandscapeKeyboardHeight

#define  OFFSET 20  //新闻图集的偏移位置

//安全释放对象
#define RELEASE_SAFELY(__POINTER) { if(__POINTER){[__POINTER release]; __POINTER = nil; }}

/**
 * @返回当前iPhone OS 运行的版本
 */
float FLYOSVersion();

/**
 * 
 */
BOOL FLYOSVersionIsAtLeast(float version);

/**
 * @返回YES 键盘是可视化的
 */
BOOL FLYIsKeyboardVisible();

/**
 * @返回YES，设备支持iPhone
 */
BOOL FLYIsPhoneSupported();

/**
 * @返回YES，设备是iPad
 */
BOOL FLYIsPad();

/**
 * @返回当前设备方向
 */
UIDeviceOrientation FLYDeviceOrientation();

/**
 * 在iPhone/iPad touch上
 * 检查如果屏幕是portrait, landscape left, or landscape right.
 * 这个方法帮你忽略home按钮朝上的方向
 *
 * 在iPad上，一直返回YES
 */
BOOL FLYIsSupportedOrientation(UIInterfaceOrientation orientation);

/**
 * @指定方法返回旋转的transform
 */
CGAffineTransform FLYRotateTransformForOrientation(UIInterfaceOrientation orientation);

/**
 * @返回应用程序物理屏幕大小
 *
 */
CGRect FLYApplicationBounds();

/**
 * @返回应用程序去掉状态栏高度的大小
 *
 */
CGRect FLYApplicationFrame();

/**
 * @返回指定方向toolBar的高度
 *
 */
CGFloat FLYToolbarHeightForOrientation(UIInterfaceOrientation orientation);

/**
 * @ 返回指定方向的键盘的高度
 */
CGFloat FLYKeyboardHeightForOrientation(UIInterfaceOrientation orientation);

/**
 * @ 返回分组表格视图cell与屏幕之间的间距，iPad的间距会更大
 */
CGFloat FLYGroupedTableCellInset();

///////////////////////////////////////////////////////////////////////////////////////////////////
// Dimensions of common iPhone OS Views

/**
 * 标准的tableview行高
 * @const 44 pixels
 */
extern const CGFloat FLYkDefaultRowHeight;

/**
 * 标准的toolBar竖屏的高度The standard height of a toolbar in portrait orientation.
 * @const 44 pixels
 */
extern const CGFloat FLYkDefaultPortraitToolbarHeight;

/**
 * 标准的toolBar横屏的高度 
 * @const 33 pixels
 */
extern const CGFloat FLYkDefaultLandscapeToolbarHeight;

/**
 * 标准的键盘竖屏的高度
 * @const 216 pixels
 */
extern const CGFloat FLYkDefaultPortraitKeyboardHeight;

/**
 * 标准的键盘横屏的高度
 * @const 160 pixels
 */
extern const CGFloat FLYkDefaultLandscapeKeyboardHeight;

/**
 * 分组的表格视图cell边界与屏幕的间距
 * @const 10 pixels
 */
extern const CGFloat FLYkGroupedTableCellInset;
